...
def canonical_cluster_column(adata, preferred: list[str]) -> Optional[str]:
    for col in preferred:
        if valid_group_column(adata, col):
            return col
    return None
...
def export_cluster_coverages(out_dir: str, atac, rna) -> None:
    ...
    rna_cluster_col = canonical_cluster_column(
        rna,
        ["cluster", "Clusters", "RNA_cluster"],
    )
    if rna_cluster_col is not None:
        groupby = "rna_Clusters"
        copy_rna_obs_to_atac(atac, rna, rna_cluster_col, groupby)
        logging.info(
            "Exporting RNA cluster coverage tracks for '%s'.",
            rna_cluster_col,
        )
        export_coverage_group(
            atac,
            groupby=groupby,
            out_dir=os.path.join(out_dir, "RNA_cluster_coverages"),
            suffix="_RNA_cluster.bw",
        )
    else:
        logging.info("No RNA cluster column found for coverage export.")

    atac_cluster_col = canonical_cluster_column(
        atac,
        ["Clusters", "cluster", "ATAC_cluster"],
    )
    if atac_cluster_col is not None:
        logging.info(
            "Exporting ATAC cluster coverage tracks for '%s'.",
            atac_cluster_col,
        )
        export_coverage_group(
            atac,
            groupby=atac_cluster_col,
            out_dir=os.path.join(out_dir, "ATAC_cluster_coverages"),
            suffix="_ATAC_cluster.bw",
        )
    else:
        logging.info("No ATAC cluster column found for coverage export.")
...
def write_archr_cluster_metadata(rna, path: str) -> None:
    ...
    if rna_cluster_col is not None:
        metadata["rna_Clusters"] = rna.obs[rna_cluster_col].astype(str).values
